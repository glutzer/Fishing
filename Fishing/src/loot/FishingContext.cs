...
public class FishingContext
{
    ...
    private void CalculateRarityQuantity(Entity? caster, float liquidBaseLine)
    {
        ...
        if (caster is EntityPlayer player)
        {
            ...
            if (hotbarSlot.Itemstack != null && hotbarSlot.Itemstack.Collectible is ItemFishingRod)
            {
                if (ItemFishingRod.ReadStack(2, hotbarSlot.Itemstack, MainAPI.Sapi, out ItemStack? baitStack))
                {
