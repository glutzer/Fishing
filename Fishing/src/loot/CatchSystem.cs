...
public class CatchSystem : GameSystem
{
    ...
    public void ConsumeBait(Entity? entity)
    {
        ...
        if (hotbarSlot.Itemstack == null || hotbarSlot.Itemstack.Collectible is not ItemFishingRod) return;
        ItemFishingRod.ReadStack(2, hotbarSlot.Itemstack, api, out ItemStack? baitStack);
        ...
        {
            ItemFishingRod.DamageStack(2, hotbarSlot, api, 1);
        }
        else if (Random.Shared.NextSingle() < collectibleBehaviorBait.consumeChance)
        {
            baitStack.StackSize--;
            ItemFishingRod.SetStack(2, hotbarSlot.Itemstack, baitStack.StackSize <= 0 ? null : baitStack);
        }
    }
}