...
public class EntityBobber : Entity, IPhysicsTickable
{
    ...
    public bool IsValid()
    {
        ...
        if (hotbarSlot.Itemstack == null || hotbarSlot.Itemstack.Collectible is not ItemFishingRod || hotbarSlot != rodSlot) return false;

        EntityBobber? bobber = ItemFishingRod.TryGetBobber(hotbarSlot, Api);
