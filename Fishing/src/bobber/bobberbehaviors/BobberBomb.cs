...
public class BobberBomb : BobberReelable
{
    ...
    public override void OnAttackStart(bool isServer, ItemSlot rodSlot, EntityPlayer player)
    {
        ...
        if (isServer && player.Controls.ShiftKey)
        {
            ...
            ItemFishingRod.ReadStack(1, rodSlot.Itemstack, MainAPI.Sapi, out ItemStack? bobberStack);

            if (bobberStack == null || bobberStack.StackSize == 1)
            {
                ItemFishingRod.SetStack(1, rodSlot.Itemstack, null);
            }
            else
            {
                bobberStack.StackSize--;
                ItemFishingRod.SetStack(1, rodSlot.Itemstack, bobberStack);
            }
