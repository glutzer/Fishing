...
public class BobberFishable : BobberReelable
{
    ...
    public void DrainDurabilityServer(float dt)
    {
        ...
        if (ItemFishingRod.DamageStack(0, bobber.rodSlot, bobber.Api, duraAccumInt))
        {
            bobber.Die();
    ...
    public override void TryCatchServer()
    {
        ...
        if (bobber.rodSlot == null || bobber.rodSlot.Itemstack == null || bobber.rodSlot.Itemstack.Collectible is not ItemFishingRod) return;

        if (bitingFish.itemStack != null) ItemFishingRod.SetStack(3, bobber.rodSlot.Itemstack, bitingFish.itemStack);
